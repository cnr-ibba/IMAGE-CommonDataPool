...
FIELD_LIST = [
    "analysis_accession", "study_accession", "sample_accession",
    "analysis_title", "analysis_type", "center_name", "first_public",
    "last_updated", "study_title", "tax_id", "scientific_name",
    "analysis_alias", "submitted_bytes", "submitted_md5", "submitted_ftp",
    "submitted_aspera", "submitted_galaxy", "submitted_format", "broker_name",
...
@click.option(
    '--result_type',
    default='specimen',
    help='Specify the result type which can be either specimen (default) or '
         'analysis'
)
def main(result_type):
    ...
    if result_type != SPECIMEN and result_type != ANALYSIS:
        print(f"The parameter result_type can only be '{SPECIMEN}' or "
              f"'{ANALYSIS}")
        exit(1)
    ...
    for study_accession in image_datasets:
        url = f"http://www.ebi.ac.uk/eva/webservices/rest/v1/studies/" \
              f"{study_accession}/summary"
        ...
        # 204 is the status code for no content => the current term does not
        # have match
...
def extract_files(record, file_server_type):
    ...
    if len(files) != len(checksums) or len(files) != len(sizes) \
            or len(files) != len(formats) or len(files) == 0:
        return es_doc
...
def parse_into_analysis(record, results, eva_summary):
    ...
    if analysis_accession in results:
        ...
        if not es_doc:
            return results
        # in ENA api, it is description in ena result, different to
        # analysis_description in faang result portal
        ...
        # use PRJEB22988 (non farm animal) as example being both imputation
        # and phasing project
...
def convert_analysis(record):
    ...
    es_doc['organism']['ontologyTerms'] = f"http://purl.obolibrary.org/obo/" \
                                          f"NCBITaxon_{record['tax_id']}"
